...
class Lady_FavoritesViewController: UIViewController {
    ...
    private func lady_createViews() {
        ...
        lady_backButton.titleLabel?.font = UIFont.boldSystemFont(ofSize: 16)
        lady_backButton.translatesAutoresizingMaskIntoConstraints = false
        lady_backButton.addTarget(self, action: #selector(lady_backButton(_:)), for: .touchUpInside)
        lady_titleView.addSubview(lady_backButton)
        
        // 创建清除按钮
        lady_clearButton = UIButton(type: .system)
        lady_clearButton.setTitle("Clear All", for: .normal)
        lady_clearButton.setTitleColor(.systemRed, for: .normal)
        ...
        NSLayoutConstraint.activate([
            ...
            // 返回按钮约束
            lady_backButton.leadingAnchor.constraint(equalTo: lady_titleView.leadingAnchor, constant: 20),
            lady_backButton.centerYAnchor.constraint(equalTo: lady_titleView.centerYAnchor),
    ...
    @objc func lady_backButton(_ sender: Any) {
...
extension Lady_FavoritesViewController: UITableViewDataSource, UITableViewDelegate {
